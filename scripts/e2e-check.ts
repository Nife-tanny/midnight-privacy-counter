/**
 * End-to-end smoke check for counter-app.
 *
...
import { getOrCreateOwnerSecret } from '../src/owner-secret';

// @ts-expect-error wallet sync requires WebSocket
globalThis.WebSocket = WebSocket;

// Must match the privateStateId used at deploy time. PS is unused ({}) —
// the counter contract's only private input is the secretKey witness.
...
async function main() {
  ...
  // See src/deploy.ts for why this is built as explicit `any` steps rather
  // than a typed .pipe() chain (TypeScript can't fully infer through the
  // dynamic `await import()` above).

/**
 * CLI for interacting with counter-app contract
 */
...
// the same private state. PS is unused ({}) — the counter contract's only
// private input is the secretKey witness, read fresh from local disk below.
...
// Same secret used at deploy time — required to pass the increment circuit's
// ownership check. Never sent on-chain; read fresh from local disk each call.
const ownerSecret = getOrCreateOwnerSecret();

// See deploy.ts for why this is built as explicit `any` steps rather than a
// typed .pipe() chain (TypeScript can't fully infer through the dynamic
// `await import()` above).
...
async function main() {
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Increment counter (requires the owner secret)');
        ...
        case '2': {
          ...
          try {
            ...
            if (contractState) {
              ...
              console.log(`\n  📋 Current count: ${ledgerState.count.toString()}\n`);
            } else {

/**
 * Deploy counter-app contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { getOrCreateOwnerSecret } from './owner-secret';
...
// counter contract keeps no meaningful private state of its own (PS = {})
// — its only private input is the `secretKey` witness, supplied fresh on
// every call from .counter-owner-secret (see owner-secret.ts).
...
// The `secretKey` witness never touches the ledger — it's read fresh from
// local disk (.counter-owner-secret) on every circuit call.
const ownerSecret = getOrCreateOwnerSecret();
const initialOwnerKey = CounterContract.pureCircuits.deriveOwnerKey(ownerSecret);

// Note: TypeScript can't fully infer the contract's generic through the
// dynamic `await import()` above (same pre-existing limitation as this
// template's "build" script, which already tolerates it: see package.json's
// `build` script). Breaking the pipe into explicit `any` steps keeps that a
// type-checker-only wrinkle rather than letting it cascade; the local
// simulator tests in tests/counter.test.ts exercise the real runtime shape.
const compiledContractBase: any = CompiledContract.make('counter', CounterContract.Contract);
const compiledContractWithWitnesses: any = (CompiledContract.withWitnesses as any)({
  secretKey: () => [{}, ownerSecret],
})(compiledContractBase);
const compiledContract: any = (CompiledContract.withCompiledFileAssets as any)(zkConfigPath)(
  compiledContractWithWitnesses,
);
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy counter-app to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState ({} — the counter contract's PS is unused,
      // its only private input is the secretKey witness). args is the
      // contract constructor's arguments: the public commitment to the
      // owner secret, computed above via deriveOwnerKey. (Statically-typed
